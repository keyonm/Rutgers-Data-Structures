...
public class RUHungry {
    ...
    public void menu(String inputFile) {
        ...
        this.menuVar = new MenuNode[len];
        this.categoryVar = new String[len];
        int i = 0;
        while (!StdIn.isEmpty()) {
            String cat = StdIn.readLine();
            this.categoryVar[i] = cat;
            ...
            for (int j = 0; j < dishes; j++) {
                ...
                StdIn.readLine();
                Dish dish = new Dish(cat, dishName, ing);
                MenuNode newNode = new MenuNode(dish, this.menuVar[i]);
                this.menuVar[i] = newNode;
            }
    ...
    public void addStockNode ( StockNode newNode ) {
        int index = newNode.getIngredient().getID() % this.stockVarSize;
        if (this.stockVar[index] == null) {
            this.stockVar[index] = newNode;
        } else {
            newNode.setNextStockNode(this.stockVar[index]);
            this.stockVar[index] = newNode;
        }
    }
    ...
    public void deleteStockNode ( String ingredientName ) {
        for (int i = 0; i < this.stockVar.length; i++) {
            StockNode prev = null;
            StockNode ptr = this.stockVar[i];
            if (ptr.getIngredient().getName().equals(ingredientName)) {
                if (ptr.getNextStockNode() == null) {
                    this.stockVar[i] = null;
                } else {
                    this.stockVar[i] = ptr.getNextStockNode();
                }
                return;
            }
            while (ptr != null) {
    ...
    public StockNode findStockNode (int ingredientID) {
        int index = ingredientID % this.stockVarSize;
        StockNode ptr = this.stockVar[index];
        while (ptr != null) {
    ...
    public void updateStock (String ingredientName, int ingredientID, int stockAmountToAdd) {
        if (ingredientName == null) {
            findStockNode(ingredientID).getIngredient().setStockLevel(findStockNode(ingredientID).getIngredient().getStockLevel() + stockAmountToAdd);
        } else if (ingredientID == -1) {
            Ingredient ing = findStockNode(ingredientName).getIngredient();
            int stock = ing.getStockLevel();
            ing.setStockLevel(stock + stockAmountToAdd);
        }
    }
    ...
    public void createStockHashTable (String inputFile){
        ...
        this.stockVar = new StockNode[this.stockVarSize];
        // WRITE YOUR CODE HERE
        while(!StdIn.isEmpty()) {
            ...
            Ingredient newIng = new Ingredient(id, name, amt, cost);
            StockNode newNode = new StockNode(newIng, null);
    ...
    public boolean checkDishAvailability ( String dishName, int numberOfDishes ){
        ...
        int[] ings = node.getDish().getStockID();
        for (int i = 0; i < ings.length; i++) {
            if (findStockNode(ings[i]).getIngredient().getStockLevel() < numberOfDishes) {
                return false;
            }
        }
    ...
    public void order ( String dishName, int quantity ){
        if (checkDishAvailability(dishName, quantity)) {
            TransactionData newNode = new TransactionData("order", dishName, quantity, findDish(dishName).getDish().getProfit() * quantity, true);
            addTransactionNode(newNode);
            int[] ids = findDish(dishName).getDish().getStockID();
            for (int i = 0; i < ids.length; i++) {
                updateStock(null, ids[i], -1 * quantity);
            }
        } else {
            TransactionData newNode = new TransactionData("order", dishName, quantity, 0, false);
            addTransactionNode(newNode);
            MenuNode ptr = findDish(dishName);
            if (ptr.getNextMenuNode() != null) {
                reOrder(ptr.getNextMenuNode().getDish().getDishName(), quantity, dishName);
            } else {
                String cat = findDish(dishName).getDish().getCategory();
                int index = -1;
                for (int i = 0; i < this.categoryVar.length; i++) {
                    if (this.categoryVar[i].equals(cat)) {
                        index = i;
                        break;
                    }
                }

                if (this.menuVar[index].getDish().getDishName().equals(dishName)) {
                    return;
                } else {
                    reOrder(this.menuVar[index].getDish().getDishName(), quantity, dishName);
                }
            }
        }
    }

    public void reOrder (String dishName, int quantity, String Original) {
        if (checkDishAvailability(dishName, quantity)) {
            TransactionData newNode = new TransactionData("order", dishName, quantity, findDish(dishName).getDish().getProfit() * quantity, true);
            addTransactionNode(newNode);
            int[] ids = findDish(dishName).getDish().getStockID();
            ...
            addTransactionNode(newNode);
            MenuNode ptr = findDish(dishName);
            if (ptr.getNextMenuNode() != null && !ptr.getNextMenuNode().getDish().getDishName().equals(Original)) {
                reOrder(ptr.getNextMenuNode().getDish().getDishName(), quantity, Original);
            } else if (ptr.getNextMenuNode() != null && ptr.getNextMenuNode().getDish().getDishName().equals(Original)) {
                return;
            } else {
                String cat = findDish(dishName).getDish().getCategory();
                int index = -1;
                ...
                if (this.menuVar[index].getDish().getDishName().equals(Original)) {
                    return;
                } else {
                    reOrder(this.menuVar[index].getDish().getDishName(), quantity, Original);
                }
            }
        }
    }
    ...
    public void donation ( String ingredientName, int quantity ){
        boolean success = profit() > 50.0 && findStockNode(ingredientName).getIngredient().getStockLevel() >= quantity;
        if (success) {
            updateStock(ingredientName, -1, -1 * quantity);
        }
        TransactionData newNode = new TransactionData("donation", ingredientName, quantity, 0, success);
        addTransactionNode(newNode);
    }
    ...
    public void restock ( String ingredientName, int quantity ){
        ...
        boolean success = node.getIngredient().getCost() * quantity <= profit();
        TransactionData restock;
        if (success) {
            updateStock(ingredientName, -1, quantity);
            restock = new TransactionData("restock", ingredientName, quantity, -1 * node.getIngredient().getCost() * quantity, true);
        } else {
